...
gs=plt.GridSpec(4,1, hspace=0.2, wspace=0.2)
...
#Title of the plot
fig.text(0.2, 0.95, "Nanoreactor(Box Size: 20x20 units", color="white",backgroundcolor="black", fontsize=16)
...
ax1.set_ylabel("Number of the species",fontsize=12)
ax1.set_xlim(0,20)
ax1.set_ylim(0,30)
...
#Time Step Counter
ts=ax1.annotate("Time Steps ={time}".format(time=tsCounter), xy=(10,25), fontsize=14)


#Initialization of plot for representing composition after 500 time step
report0=ax2.annotate("After {timestep} Time Step(s)".format(timestep=0), xy=(0.3,0.8), fontsize=12)
report0.set_color('brown')
report1=ax2.annotate("Number of O radical(s) ={oxygen}".format(oxygen=0), xy=(0.3,0.6))
report1.set_color('red')
report2=ax2.annotate("Number of O2 molecule(s) ={oxygen}".format(oxygen=20), xy=(0.3,0.4))
report2.set_color('green')
report3=ax2.annotate("Number of O3 molecule(s) ={oxygen}".format(oxygen=0), xy=(0.3,0.2))
...
def update2(c500):
	...
	return report0,report1,report2,report3
